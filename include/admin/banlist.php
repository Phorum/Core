...
$ban_types = array(
    PHORUM_BAD_IPS        => "IP Address/Hostname",
    PHORUM_BAD_NAMES      => "Name/User Name",
    PHORUM_BAD_EMAILS     => "Email Address",
    PHORUM_BAD_USERID     => "User-Id (registered User)",
    PHORUM_BAD_SPAM_WORDS => "Illegal Words (SPAM)"
);
...
if($_GET["curr"] && $_GET["delete"]){
    ...
    $row = $frm->addrow(
        'Comments',
        $frm->textarea('comments', htmlspecialchars($comments), 50, 7)
    );
        ...
        if(count($PHORUM['banlists'])){
            ...
            foreach($PHORUM["banlists"] as $type => $content){
                ...
                foreach($content as $key => $item){
                    $edit_url = phorum_api_admin_url(array('module=banlist','edit=1',"curr=$key"));
                    $delete_url = phorum_api_admin_url(array('module=banlist','delete=1',"curr=$key"));
