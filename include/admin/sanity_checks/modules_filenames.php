    ...
    while (false !== ($entry = $d->read()))
    {
        ...
        if (is_file("./mods/$entry") && substr($entry, -4)==".php") {
            ...
            if (!file_exists("./mods/$entry/info.txt")) {
                return array(
                    PHORUM_SANITY_WARN,
                    "Your module &quot;$qentry&quot; doesn not have an info.txt
                     file in its directory. Either its not a module or the
                     installation of that module is broken.",
                    "You should remove all files or directories which are not
                     modules from the mods-directory and fix broken module
                     installations."
                );
            } elseif (!file_exists("./mods/$entry/$entry.php")) {
                return array(
                    PHORUM_SANITY_WARN,
                    "Your module &quot;$qentry&quot; doesn not have an
                     corresponding .php file in its directory. Each module
                     needs a .php file with the same name as the directory
                     to work. Either that directory is not for a module or
                     the installation is broken and either the filename or
                     the directoryname needs to be corrected.",
                    "You should remove all files or directories which are not
                     modules from the mods-directory and fix broken module
                     installations."
                );
