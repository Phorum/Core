...
$frm->addrow("Generate updated version of an existing language file",
             $frm->select_tag("language", phorum_get_language_info(), $language));
$frm->show();
...
function phorum_get_language($lang)
{
    ...
    $DEPRECATED = array();
    $keep_comment = '';
    if (! file_exists($path)) phorum_user_error(
            ...
            if ($token[0] == T_VARIABLE) {
                ...
                if ($tokens[0][0] == T_ARRAY)
                {
                    ...
                    while (count($tokens))
                    {
                        ...
                        if ($endedby != ',' && $endedby != ')') phorum_user_error(
...
function phorum_extract_language_strings_recurse($path)
{
    ...
    while (($f = readdir($dh)))
    {
        ...
        if (preg_match('/\.(php|tpl)$/', $file)) {
            $fp = fopen($file, "r");
            if (! $fp) phorum_user_error("Can't read file '$file'");
