...
function phorum_db_delete_message($message_id, $mode = PHORUM_DELETE_MESSAGE)
{
    ...
    if (empty($rec)) {
        phorum_db_mysql_error("No message found for message_id $message_id");
    }
...
function phorum_db_newflag_get_unread_count($forum_id=0)
{
    ...
    // Get user read thread count
    $sql = "select sql_calc_found_rows phorum_user_newflags.message_id as min_thread from phorum_user_newflags inner join phorum_messages using (message_id) where phorum_user_newflags.user_id={$PHORUM['user']['user_id']} and phorum_user_newflags.forum_id in ({$forum_id}) and parent_id=0 and status=2 order by phorum_user_newflags.message_id limit 1";
    $res = mysql_query($sql, $conn);
    if ($err = mysql_error()) phorum_db_mysql_error("$err: $sql");
    if(mysql_num_rows($res)){
        $min_thread = (int)mysql_result($res, 0, "min_thread");
    } else {
        $min_thread=0;
    }

    $sql = "select found_rows() as read_thread_count";
    $res = mysql_query($sql, $conn);
    if ($err = mysql_error()) phorum_db_mysql_error("$err: $sql");
    $read_thread_count = (int)mysql_result($res, 0, "read_thread_count");

    //Get user read messages:
    $sql = "select sql_calc_found_rows phorum_user_newflags.message_id as min_message_id from phorum_user_newflags inner join phorum_messages using (message_id) where phorum_user_newflags.user_id={$PHORUM['user']['user_id']} and phorum_user_newflags.forum_id in ({$forum_id}) and status=2 order by phorum_user_newflags.message_id limit 1";
    $res = mysql_query($sql, $conn);
    if ($err = mysql_error()) phorum_db_mysql_error("$err: $sql");
    if(mysql_num_rows($res)){
    ...
    $sql = "select found_rows() as read_message_count";
    $res = mysql_query($sql, $conn);
    if ($err = mysql_error()) phorum_db_mysql_error("$err: $sql");
    $read_message_count = (int)mysql_result($res, 0, "read_message_count");

    if($min_message_id>0){

        // Get thread count
        $sql = "select count(*) as thread_count from phorum_messages where forum_id in ({$forum_id}) and message_id>=$min_message_id and status=2 and parent_id=0";
        $res = mysql_query($sql, $conn);
        if ($err = mysql_error()) phorum_db_mysql_error("$err: $sql");
        $thread_count = (int)mysql_result($res, 0, "thread_count");

        // Get message count
        $sql = "select count(*) as msg_count from phorum_messages where forum_id in ({$forum_id}) and message_id>=$min_message_id and status=2";
        $res = mysql_query($sql, $conn);
        if ($err = mysql_error()) phorum_db_mysql_error("$err: $sql");
        $msg_count = (int)mysql_result($res, 0, "msg_count");
...
function phorum_db_create_tables()
{
    ...
    $queries = array(
        ...
        // create tables
        "CREATE TABLE {$PHORUM['forums_table']} ( forum_id int(10) unsigned NOT NULL auto_increment, name varchar(50) NOT NULL default '', active smallint(6) NOT NULL default 0, description text NULL, template varchar(50) NOT NULL default '', folder_flag tinyint(1) NOT NULL default '0', parent_id int(10) unsigned NOT NULL default '0', list_length_flat int(10) unsigned NOT NULL default '0', list_length_threaded int(10) unsigned NOT NULL default '0', moderation int(10) unsigned NOT NULL default '0', threaded_list tinyint(4) NOT NULL default '0', threaded_read tinyint(4) NOT NULL default '0', float_to_top tinyint(4) NOT NULL default '0', check_duplicate tinyint(4) NOT NULL default '0', allow_attachment_types varchar(100) NOT NULL default '', max_attachment_size int(10) unsigned NOT NULL default '0', max_totalattachment_size int(10) unsigned NOT NULL default '0', max_attachments int(10) unsigned NOT NULL default '0', pub_perms int(10) unsigned NOT NULL default '0', reg_perms int(10) unsigned NOT NULL default '0', display_ip_address smallint(5) unsigned NOT NULL default '1', allow_email_notify smallint(5) unsigned NOT NULL default '1', language varchar(100) NOT NULL default 'english', email_moderators tinyint(1) NOT NULL default '0', message_count int(10) unsigned NOT NULL default '0', sticky_count int(10) unsigned NOT NULL default '0', thread_count int(10) unsigned NOT NULL default '0', last_post_time int(10) unsigned NOT NULL default '0', display_order int(10) unsigned NOT NULL default '0', read_length int(10) unsigned NOT NULL default '0', vroot int(10) unsigned NOT NULL default '0', edit_post tinyint(1) NOT NULL default '1',template_settings text NULL, count_views tinyint(1) unsigned NOT NULL default '0', display_fixed tinyint(1) unsigned NOT NULL default '0', reverse_threading tinyint(1) NOT NULL default '0',inherit_id int(10) unsigned NULL default NULL, PRIMARY KEY (forum_id), KEY name (name), KEY active (active,parent_id), KEY group_id (parent_id)) TYPE=MyISAM",
        ...
        "CREATE TABLE {$PHORUM['files_table']} ( file_id int(11) NOT NULL auto_increment, user_id int(11) NOT NULL default '0', filename varchar(255) NOT NULL default '', filesize int(11) NOT NULL default '0', file_data mediumtext NOT NULL, add_datetime int(10) unsigned NOT NULL default '0', message_id int(10) unsigned NOT NULL default '0', link varchar(10) NOT NULL default '', PRIMARY KEY (file_id), KEY add_datetime (add_datetime), KEY message_id_link (message_id,link)) TYPE=MyISAM",
        ...
        "CREATE TABLE {$PHORUM['search_table']} ( message_id int(10) unsigned NOT NULL default '0', forum_id int(10) unsigned NOT NULL default '0',search_text mediumtext NOT NULL, PRIMARY KEY  (message_id), KEY forum_id (forum_id), FULLTEXT KEY search_text (search_text) ) TYPE=MyISAM",
        "CREATE TABLE {$PHORUM['user_custom_fields_table']} ( user_id INT DEFAULT '0' NOT NULL , type INT DEFAULT '0' NOT NULL , data TEXT NULL, PRIMARY KEY ( user_id , type )) TYPE=MyISAM",
        "CREATE TABLE {$PHORUM['pm_messages_table']} ( pm_message_id int(10) unsigned NOT NULL auto_increment, from_user_id int(10) unsigned NOT NULL default '0', from_username varchar(50) NOT NULL default '', subject varchar(100) NOT NULL default '', message text NOT NULL, datestamp int(10) unsigned NOT NULL default '0', meta mediumtext NULL, PRIMARY KEY(pm_message_id)) TYPE=MyISAM",
