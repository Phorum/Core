...
if(empty($PHORUM["open_id"])){
    phorum_redirect_by_url(phorum_get_url(PHORUM_URL_LOGIN));
}
...
if(isset($_POST["openid"])){
    ...
    if (!$auth_request) {
        ...
        if ($auth_request->shouldSendRedirect()) {
            $redirect_url = $auth_request->redirectURL($PHORUM["http_path"],
                                                       phorum_get_url(PHORUM_OPENID_URL, "forum_id=".$PHORUM["forum_id"]));
    ...
    $response = $consumer->complete(phorum_get_url(PHORUM_OPENID_URL, "forum_id=".$PHORUM["forum_id"]));
    ...
    if ($response->status == Auth_OpenID_CANCEL) {
        ...
        if(empty($user_id) || empty($user) || $user["active"]!=PHORUM_USER_ACTIVE){
            ...
            phorum_redirect_by_url(phorum_get_url(PHORUM_URL_INDEX));
