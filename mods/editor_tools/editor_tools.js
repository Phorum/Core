...
// This array will be filled by javascript that will be generated
...
var editor_tools_subject_smileys = new Array();
...
// The default height for our icons. This one is filled from javascript
// that will be generated by the PHP code.
...
function editor_tools_construct()
{
    ...
    for (var i = 0; i < editor_tools.length; i++)
    {
        ...
        img_obj.className = "editor_tools_button";
        img_obj.src = icon;
        ...
        // A special tool: subject_smiley. This one is added to the
...
function editor_tools_add_tags(pre, post, target)
{
    var text;
    var pretext;
    var posttext;
...
function editor_tools_handle_subject_smiley()
{
    // Create the smiley picker on first access.
    if (!editor_tools_subject_smiley_picker_obj)
    {
        // Create a new popup.
        var popup = editor_tools_construct_popup('editor_tools_subject_smiley_picker','l');
        editor_tools_subject_smiley_picker_obj = popup[0];
        var content_obj = popup[1];

        editor_tools_subject_smiley_picker_obj.style.width = editor_tools_smileys_popupwidth;

        // Populate the new popup.
        var i = 0;
        for (var smiley in editor_tools_subject_smileys)
        {
            var a_obj = document.createElement('a');
            a_obj.href = 'javascript:editor_tools_handle_subject_smiley_select("' + smiley + '")';
            var img_obj = document.createElement('img');
            img_obj.src = editor_tools_subject_smileys[smiley];
            a_obj.appendChild(img_obj);
            content_obj.appendChild(a_obj);
        }

        // Register the popup with the editor tools.
        editor_tools_register_popup_object(editor_tools_subject_smiley_picker_obj);
    }

    // Display the popup.
    var button_obj = document.getElementById('editor_tools_img_subject_smiley');
    editor_tools_toggle_popup(editor_tools_subject_smiley_picker_obj, button_obj);
}

// Called by the size picker library.
function editor_tools_handle_subject_smiley_select(smiley)
{
    smiley = editor_tools_strip_whitespace(smiley);
