...
function spamhurdle_javascript_signature_check_form($data)
{
    ...
    if (!isset($_POST[$fld]))
    {
        ...
        spamhurdles_log(
            "Spam Hurdles blocked post, form id \"{$data['id']}\"",
            "The javascript signature spam hurdle was enabled for the " .
            "form, but the posted form data does not contain the " .
            "expected signature field \"$fld\". Possibly, the user " .
            "does not have JavaScript enabled in his browser."
        );
    }
    ...
    else
    {
        ...
        if ($sig != $expected_sig)
        {
            ...
            spamhurdles_log(
                "Spam Hurdles blocked post, form id \"{$data['id']}\"",
                "The browser returned an incorrect signature for the " .
                "javascript signature spam hurdle. The expected signature " .
                "was \"$expected_sig\", but the browser return \"$sig\". "
            );
        }
    }
