...
<span class="PhorumNavHeading PhorumHeadingLeft">{LANG->Options}:</span>&nbsp;<a class="PhorumNavLink" href="{MESSAGE->reply_url}">{LANG->Reply}</a>&bull;<a class="PhorumNavLink" href="{MESSAGE->quote_url}">{LANG->QuoteMessage}</a>&bull;{IF LOGGEDIN}<a class="PhorumNavLink" href="{MESSAGE->follow_url}">{LANG->FollowThread}</a>&bull;{/IF}<a class="PhorumNavLink" href="javascript:if(window.confirm('{LANG->ConfirmReportMessage}')) window.location='{MESSAGE->report_url}';">{LANG->Report}</a>{if MESSAGE->edit 1}&bull;<a class="PhorumNavLink" href="{MESSAGE->edituser_url}">{LANG->EditPost}</a>{/if}
</div>
