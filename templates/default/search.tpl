...
{POST_VARS}
<div class="PhorumFloatingText" style="float: right; padding: 10px; text-align; left; width:50%">
{LANG->SearchAuthors}:<br /><input type="text" name="search" size="30" maxlength="" value="{SEARCH->safe_search}" />&nbsp;<input type="submit" value="{LANG->Search}" /><br />
...
<div style="width: 45%; border-right: 1px solid #C3BEBA; padding: 10px;">
<form action="{URL->ACTION}" method="get" style="display: inline;">
<input type="hidden" name="forum_id" value="{SEARCH->forum_id}" />
{POST_VARS}
{LANG->SearchMessages}:<br /><input type="text" name="search" size="30" maxlength="" value="{SEARCH->safe_search}" />&nbsp;<input type="submit" value="{LANG->Search}" /><br />
<div style="margin-top: 3px;"><select name="match_forum"><option value="ALL" {if SEARCH->match_forum ALL}selected{/if}>{LANG->MatchAllForums}</option>{if SEARCH->allow_match_one_forum}<option value="THISONE" {if SEARCH->match_forum THISONE}selected{/if}>{LANG->MatchThisForum}</option>{/if}</select></div>
